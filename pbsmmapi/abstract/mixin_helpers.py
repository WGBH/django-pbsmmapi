...
#     2. An Episode whose parent Season isn't available (or that Season's Show)
#         is LIKEWISE not available EVEN IF the object's settings say otherwise.
#
# This (so far) does NOT apply to Detail pages, only listing pages.
# Why?  Because there might be a very fringe case where you have an
# Episode/Special whose detail page you DO want available but out of context of
# the overall framework.
#
# In this situation, you COULD link directly to the affected page from elsewhere
# and it would appear, even though it would not appear on the object's listing
# page.
...
def filter_offline_shows(queryset, is_logged_in):
    """
    This is used by the PBSMMSpecialListView and PBSMMSeasonListView to
    self-filter objects whose parents are not online. (Unless you're logged
    into the Admin.)
    """
...
def filter_offline_seasons(queryset, is_logged_in):
    """
    This is used by the PBSMMEpisodeListView to self-filter objects whose
    parents are not online. (Unless you're logged into the Admin)
    """
    ...
    # If you're logged in then all remaining objects are available to you at
    # this point. Otherwise there are still two other cases to check;  you're
    # parental Season and grand-parental Show must be available to you.
    if not is_logged_in:
        now = datetime.now(pytz.utc)
...
def filter_offline_parent_show(obj, user):
    """
    This is for Specials and Seasons - if my Show is not available, than I am
    likewise not available. Remember that "Available" also depends on whether
...
def filter_offline_parent_season(obj, user):
    """
    This is for Episodes - if my Season or my Season's Show are not available,
    then I am likewise not available. Remember that "Available" also depends on
    whether you are logged into the Admin.
    """
