...
#        1. (publish_status =  1):
#            Object is PERMANANTLY LIVE - think of this as a ALWAYS ON switch
#        2. (publish_status =  0):
#            Object is CONDITIONALLY LIVE - the "live_as_of" date is in the past
#        3. (publish_status =  0):
#            Object is PENDING LIVE - the "live_as_of" date is still in the future
#        4. (publish_status =  0):
#            Object is NOT YET PUBLISHED - the "live_as_of" date is NULL
#        4. (publish_status = -1):
#            Object is PERMANENYLU OFFLINE - this is the ALWAYS OFF Switch
#
# Page requesters can either be logged in to the Django Admin (i.e., staff) or
# not (i.e., the general public)
#
#    RULE 1:   Objects - when created start off with publish_status = 0 and
#    live_as_of = None. This means that the logged-in Admin user can edit the
#    record and "see" the page for Q/A, but it is NOT AVAILABLE to the public.
#
...
def can_object_page_be_shown(user, this_object):
    if getattr(user, 'is_staff', False):  # admin users can always see pages
    ...
    if this_object.publish_status == 1:  # this object is live
        return True
    if this_object.publish_status == 0 and this_object.live_as_of is not None:
