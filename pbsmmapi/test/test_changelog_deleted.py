...
class ChangelogDeletedTestCase(TestCase):
    def make_show(self, slug="nova", object_id=SHOW_ID):
        show = Show(slug=slug, object_id=UUID(object_id))
    ...
    def make_show_tree(self):
        ...
        special = Special(
            ...
            show_api_id=UUID(SHOW_ID),
            title="A Special",
            object_id=UUID(SPECIAL_ID),
        )
        special.save(skip_ingest=True)
        show_asset = Asset(
            slug="a-show-asset",
            show=show,
            object_id=UUID(SHOW_ASSET_ID),
        )
        show_asset.save()
        episode_asset = Asset(
            slug="an-episode-asset",
            episode=episode,
            object_id=UUID(EPISODE_ASSET_ID),
        )
        episode_asset.save()
    ...
    def test_delete_entry_marks_object_and_changelog(self):
        ...
        self.assertEqual(show.deleted, parse_changelog_timestamp(T2))
    ...
    def test_newer_update_clears_deleted(self):
        ...
        show.refresh_from_db()
        self.assertIsNotNone(show.deleted)
        ...
        show.refresh_from_db()
        self.assertIsNone(show.deleted)
        log.refresh_from_db()
    ...
    def test_latest_entry_wins(self):
        ...
        sync_deleted_state(log)
        show.refresh_from_db()
        self.assertEqual(show.deleted, parse_changelog_timestamp(T4))
    ...
    def test_cascade_marks_descendants(self):
        show, season, episode, special, show_asset, episode_asset = (
            self.make_show_tree()
        )
        # episode_asset was already deleted on its own, earlier
        Asset.objects.filter(pk=episode_asset.pk).update(
            deleted=parse_changelog_timestamp(T0)
        )
        ...
        for item in (show, season, episode, special, show_asset):
            item.refresh_from_db()
            self.assertEqual(item.deleted, deleted_at)
        episode_asset.refresh_from_db()
        self.assertEqual(episode_asset.deleted, parse_changelog_timestamp(T0))

    def test_undelete_clears_cascade_marked_only(self):
        show, season, episode, special, show_asset, episode_asset = (
            self.make_show_tree()
        )
        Asset.objects.filter(pk=episode_asset.pk).update(
        ...
        # individually deleted asset is not resurrected by the cascade clear
        episode_asset.refresh_from_db()
        self.assertEqual(episode_asset.deleted, parse_changelog_timestamp(T0))

    def test_reingest_skips_deleted_objects(self):
        show = self.make_show()
        Show.objects.filter(pk=show.pk).update(deleted=parse_changelog_timestamp(T1))
    ...
    def test_save_does_not_ingest_deleted(self):
        show = self.make_show()
        show.refresh_from_db()
        show.deleted = parse_changelog_timestamp(T1)
    ...
    def test_force_reingest_undeletes(self):
        ...
        with mock.patch(MMAPI_GET_URL, side_effect=mocked_requests_get):
            show_admin = PBSMMShowAdmin(Show, django_admin.site)
            show_admin.force_reingest(None, Show.objects.filter(pk=show.pk))

        show.refresh_from_db()
        self.assertIsNone(show.deleted)
        self.assertEqual(show.last_api_status, 200)
        self.assertEqual(show.object_id, UUID(SHOW_ID))
    ...
    def test_get_changelog_data_skips_deleted_logs(self):
        ...
        self.assertNotIn(deleted_log.content_id, fetched_ids)
        self.assertIn(live_log.content_id, fetched_ids)
    ...
    def test_backfill_deleted_command(self):
        show = self.make_show()
        show2 = self.make_show(slug="nature", object_id=SHOW2_ID)
        ...
        self.assertIsNone(show2.deleted)
